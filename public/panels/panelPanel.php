...
<script> 
   ...
   YAHOO.util.Event.addListener(window, "load", function() {
      ...
      dataSource = new YAHOO.util.DataSource("<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/panels.php?");
   ...
   function addPanel() {
      ...
      if (formObject.id.value.trim().length == 0)
         ...
         var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/panels.php", {success:addPanelCallback});
      }
   }
   ...
   function deletePanelsConfirm(list) {
      ...
      var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/panels.php", {success:deletePanelsCallback}, postdata);
   }
   ...
   function recoverDataRolsLists(panel_id, panel_text) {
      ...
      emptyList("unassigned_rols");
      emptyList("assigned_rols");
      var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/panels.php", {success:rolListCallback, argument:["unassigned_rols"]}, "op=listUnassignedRols&panel="+panel_id);
      var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/panels.php", {success:rolListCallback, argument:["assigned_rols"]}, "op=listAssignedRols&panel="+panel_id);   
   }
   ...
   function assignRols() {
      if (panel_list.getMenu().activeItem != null) {
         ...
         var parseList = function(listName) {
          ...
          var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/panels.php", {success:assignRolsCallback, argument:[panel]}, "op=assignRols&panel="+panel+"&list="+list);
       }
   }
