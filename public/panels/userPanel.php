...
<script> 
   ...
   YAHOO.util.Event.addListener(window, "load", function() {
      ...
      columnDefs = [ 
	   ...
	   dataSource = new YAHOO.util.DataSource("<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/users.php?");
   ...
   function addUser() {
      ...
      if (formObject.user.value.trim().length == 0 || formObject.password.value.trim().length == 0)
         ...
         var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/users.php", {success:addUserCallback});
      }
   }
   ...
   function deleteUsersConfirm(list) {
      ...
      var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/users.php", {success:deleteUsersCallback}, postdata);
   }
...
YAHOO.extend(DDList, YAHOO.util.DDProxy, {
   ...
   function recoverDataGroupsLists(userId, userName) {
      ...
      emptyList("unassigned_groups");
      emptyList("assigned_groups");
      var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/users.php", {success:groupListCallback, argument:["unassigned_groups"]}, "op=listUnassignedGroups&user="+userId);
      var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/users.php", {success:groupListCallback, argument:["assigned_groups"]}, "op=listAssignedGroups&user="+userId);   
   }
   ...
   function assignGroups() {
      ...
      if (userName != "Select user") {
         var parseList = function(listName) {
          ...
          var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/users.php", {success:assignGroupsCallback, argument:[userId, userName]}, "op=assignGroups&user="+userId+"&list="+list);
       }
   }
