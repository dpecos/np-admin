...
<script> 
   ...
   YAHOO.util.Event.addListener(window, "load", function() {
	  ...
	  dataSource = new YAHOO.util.DataSource("<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/groups.php?");
   ...
   function addGroup() {
      ...
      if (formObject.group_name.value.trim().length == 0)
         ...
         var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/groups.php", {success:addGroupCallback});
      }
   }
   ...
   function deleteGroupsConfirm(list) {
      ...
      var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/groups.php", {success:deleteGroupsCallback}, postdata);
   }
...
YAHOO.extend(DDList, YAHOO.util.DDProxy, {
   ...
   function recoverDataUsersLists(groupId, groupName) {
      ...
      emptyList("unassigned_users");
      emptyList("assigned_users");
      var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/groups.php", {success:userListCallback, argument:["unassigned_users"]}, "op=listUnassignedUsers&group_id="+groupId);
      var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/groups.php", {success:userListCallback, argument:["assigned_users"]}, "op=listAssignedUsers&group_id="+groupId);   
   }
   ...
   function assignUsers() {
      ...
      if (groupName != "Select group") {
         var parseList = function(listName) {
          ...
          var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/groups.php", {success:assignUsersCallback, argument:[groupId, groupName]}, "op=assignUsers&group_id="+groupId+"&list="+list);
       }
   }
