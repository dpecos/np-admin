...
<script> 
   ...
   YAHOO.util.Event.addListener(window, "load", function() {
      ...
      var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/menus.php", {success:menuListCallback}, postdata);
      
      group_list = new YAHOO.widget.Button("group_list", {
            type: "menu",  
            menu: "group_list_select"
      }); 
      var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/groups.php", {success:groupListCallback}, postdata);
      var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/panels.php", {success:panelListCallback}, postdata);
   });
   ...
   function addMenu() {
      ...
      /*if (formObject.group_name.value.trim().length == 0)
         ...
         var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/menus.php", {success:addMenuCallback});
      //}
   }
   ...
   function loadNodeData(node, fnLoadComplete)  { 
      ...
      var transaction = YAHOO.util.Connect.asyncRequest('POST', "<?= npadmin_setting('NP-ADMIN', 'BASE_URL') ?>/public/ajax/menus.php", {success:function(response) {
         var menus = YAHOO.lang.JSON.parse(response.responseText);
